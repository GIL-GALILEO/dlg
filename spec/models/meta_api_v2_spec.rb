...
describe MetaApiV2 do
  ...
  context 'collection method' do
    ...
    it 'returns holding institution information' do
      expect(collection.holding_institutions).to respond_to 'display_name'
      expect(collection.holding_institutions).to respond_to 'image'
    end
    it 'returns holding collection resource information' do
      expect(collection.collection_resources).to respond_to 'slug'
      expect(collection.collection_resources).to respond_to 'title'
      expect(collection.collection_resources).to respond_to 'position'
    end
  end
end